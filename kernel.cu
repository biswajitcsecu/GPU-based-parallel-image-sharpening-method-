...
int divUp(int a, int b) { return a > b ? (a + b - 1) / b : (a + b - 1) / a ; }
...
void sharpenKernel(uchar4 *d_out, const uchar4 *d_in,const float *d_filter, int w, int h) {
    ...
    for (int rd = -RAD; rd <= RAD; ++rd) {
        for (int cd = -RAD; cd <= RAD; ++cd) {
            ...
            rgb[0]  = rgb[0] + d_filter[fltIdx]*d_in[imgIdx].x;
            rgb[1]  = rgb[1] + d_filter[fltIdx]*d_in[imgIdx].y;
            rgb[2]  = rgb[2] + d_filter[fltIdx]*d_in[imgIdx].z;
        }
    }
...
void sharpenParallel(uchar4 *arr, int w, int h) {
    const int fltSz = 2 * RAD + 1;
    const float filter[9] = {-1.0, -1.0, -1.0,-1.0, 9.0, -1.0,-1.0, -1.0, -1.0};
